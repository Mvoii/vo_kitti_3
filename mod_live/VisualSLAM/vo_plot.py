...
ax.set_title("Live 3D VO Trajectory")
...
line, = ax.plot([], [], [], color='blue', linewidth=2, label="Estimated Path")
ax.legend()

# Set starting pose
cur_x, cur_y, cur_z = 0.0, 0.0, 0.0

# 3. Your VO Processing Loop
...
print("Starting live trajectory plotting...")
...
try:
    for i, gt_pose in enumerate(tqdm(vo.gt_poses, unit="pose")):
        ...
        # Extract translations from the 4x4 matrix
        # KITTI frame: X = right, Y = down, Z = forward
        tx = cur_pose[0, 3]
        ty = cur_pose[1, 3] 
        tz = cur_pose[2, 3]

        # Map to 3D plot coordinates
        plot_x = tx
        plot_y = tz   # Forward movement becomes the Y axis on the graph
        plot_z = -ty  # Negative Y (Up) becomes the Z axis on the graph

        # Append to history
        x_hist.append(plot_x)
        y_hist.append(plot_y)
        z_hist.append(plot_z)

        # 4. Update the graph in real-time
        line.set_data(x_hist, y_hist)
        line.set_3d_properties(z_hist)

        # Fix Matplotlib 3D autoscaling bug by manually enforcing limits
        padding = 10  # meters of padding around the current trajectory
        ax.set_xlim(min(x_hist) - padding, max(x_hist) + padding)
        ax.set_ylim(min(y_hist) - padding, max(y_hist) + padding)
        
        # Z axis usually has less variance on the ground, pad it slightly differently
        z_min, z_max = min(z_hist), max(z_hist)
        if abs(z_max - z_min) < 1.0:
            ax.set_zlim(-5, 5) # Keep the Z axis from snapping too tight if the robot is flat
        else:
            ax.set_zlim(z_min - 2, z_max + 2)
        
        # Flush GUI events (this redraws the screen instantly)
        fig.canvas.draw()
        fig.canvas.flush_events()

except KeyboardInterrupt:
    # This catches the ^C if you want to stop it early without throwing a messy traceback
    print("\nProcessing interrupted by user.")

# Keep window open at the end gracefully
plt.ioff()
print("Processing complete. Close the plot window to exit.")
plt.show()
