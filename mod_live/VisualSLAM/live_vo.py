...
# 2. Setup the Plot
plot = figure(title="Live VO Trajectory (Barebones)", match_aspect=True, 
              width=800, height=600)
plot.line('x', 'y', source=source, line_width=2, color='blue')
plot.circle('x', 'y', source=source, size=4, color='red')

# 3. The Queue to pass data safely between threads
data_queue = Queue()
...
# 4. Your VO Processing Loop (Runs in the background)
def run_vo_pipeline():
    # Set initial pose
    cur_x, cur_y = 0.0, 0.0
    ...
    for i, gt_pose in enumerate(tqdm(vo.gt_poses, unit="pose")):
        ...
        # Pass the new coordinates to the queue
        data_queue.put(dict(x=[cur_pose[0, 3]], y=[cur_pose[2, 3]]))
...
# Start the VO thread
vo_thread = Thread(target=run_vo_pipeline)
vo_thread.daemon = True
vo_thread.start()

# 5. The Bokeh Update Callback
def update_plot():
    # Drain the queue and push new points to the browser
    while not data_queue.empty():
        new_pose = data_queue.get()
        # .stream() is the secret to low-overhead. It only sends the delta.
        source.stream(new_pose, rollover=5000) 

# Tell Bokeh to check for updates every 50 milliseconds
