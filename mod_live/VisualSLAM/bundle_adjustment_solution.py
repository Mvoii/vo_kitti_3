...
def objective(params, cam_param, n_cams, n_Qs, cam_idxs, Q_idxs, qs):
    ...
    # Check if the input 'params' contains only Qs (used in sparsity optimization/initialization) 
    # or Cams + Qs (used in non-sparsity optimization/initialization).
    if params.size == n_Qs * 3:
        # Case 1: Params contains only Qs (expected size 27753 in large problem)
        Qs = params.reshape((n_Qs, 3))
    else:
        # Case 2: Params contains Cams + Qs (expected size 28203 in large problem)
        # We skip the camera parameters by slicing
        Qs = params[n_cams * 9:].reshape((n_Qs, 3))
...
def bundle_adjustment(cam_params, Qs, cam_idxs, Q_idxs, qs):
    ...
    residual_init = objective(params, cam_params.ravel(), cam_params.shape[0], Qs.shape[0], cam_idxs, Q_idxs, qs)
...
def run_BA():
    ...
    small_residual_init, small_residual_minimized, opt_params = bundle_adjustment(cam_params_small, Qs_small,cam_idxs_small, Q_idxs_small, qs_small)
